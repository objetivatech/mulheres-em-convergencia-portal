...
- Pagamento é registrado (status = 'pending') → Cria notificação in-app
- Pagamento é confirmado (status = 'paid') → Cria notificação in-app

#### Trigger: `trigger_process_ambassador_payout_paid`
Dispara automaticamente quando pagamento é confirmado:
- Atualiza `total_earnings` (soma o valor pago)
- Atualiza `pending_commission` (subtrai o valor pago)

#### Trigger: `trigger_process_ambassador_payout_refund`
Dispara automaticamente quando pagamento é estornado:
- Reverte `total_earnings` (subtrai o valor estornado)
- Reverte `pending_commission` (soma o valor de volta)
