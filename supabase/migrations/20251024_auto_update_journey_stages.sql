-- Migração para atualização automática dos estágios da jornada do usuário
...
-- Criar trigger no subscriptions
DROP TRIGGER IF EXISTS trigger_update_journey_on_subscription ON public.subscriptions;
...
-- =============================================================================
-- TRIGGER 4: Atualizar para 'payment_pending' ou 'payment_confirmed'
-- =============================================================================

CREATE OR REPLACE FUNCTION public.update_journey_on_payment_status()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
BEGIN
  ...
  -- Determinar novo estágio baseado no status do pagamento
  IF NEW.status = 'pending' THEN
    new_stage := 'payment_pending';
  ELSIF NEW.status IN ('confirmed', 'received') THEN
    new_stage := 'payment_confirmed';
  ELSE
    -- Para outros status, não atualizar
    RETURN NEW;
  END IF;

  -- Atualizar jornada
  IF subscription_user_id IS NOT NULL THEN
    PERFORM public.update_user_journey_stage(
...
-- Criar trigger no payments
DROP TRIGGER IF EXISTS trigger_update_journey_on_payment ON public.payments;
CREATE TRIGGER trigger_update_journey_on_payment
  AFTER INSERT OR UPDATE OF status ON public.payments
  FOR EACH ROW
  EXECUTE FUNCTION public.update_journey_on_payment_status();
...
BEGIN
  ...
  FOR user_record IN
    SELECT DISTINCT ON (ujt.user_id)
      ...
      (SELECT COUNT(*) FROM public.subscriptions WHERE user_id = ujt.user_id) as subscription_count,
      (SELECT status FROM public.payments WHERE subscription_id IN (SELECT id FROM public.subscriptions WHERE user_id = ujt.user_id) ORDER BY created_at DESC LIMIT 1) as latest_payment_status
  ...
  LOOP
    ...
    ELSIF user_record.latest_payment_status IN ('confirmed', 'received') THEN
      should_be_stage := 'payment_confirmed';
      reason_text := format('Payment status: %s', user_record.latest_payment_status);
    ELSIF user_record.latest_payment_status = 'pending' THEN
      should_be_stage := 'payment_pending';
      reason_text := 'Payment pending';
...
COMMENT ON FUNCTION public.update_journey_on_payment_status() IS 'Trigger function: Atualiza jornada baseado no status do pagamento';
