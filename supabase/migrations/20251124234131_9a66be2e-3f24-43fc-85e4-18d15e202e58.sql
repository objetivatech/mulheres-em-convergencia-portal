-- Migração: Sincronizar categorias enum -> category_id
-- Mapeia o campo enum 'category' para o novo campo 'category_id' da tabela categories

-- Função para sincronizar categorias automaticamente
...
SELECT sync_business_category_id();

-- Criar trigger para sincronizar automaticamente quando um negócio for inserido ou atualizado
...
-- Criar trigger
DROP TRIGGER IF EXISTS trigger_auto_sync_category_id ON businesses;
CREATE TRIGGER trigger_auto_sync_category_id
  BEFORE INSERT OR UPDATE ON businesses
  FOR EACH ROW
  EXECUTE FUNCTION auto_sync_category_id();

-- Comentário para documentação
COMMENT ON FUNCTION sync_business_category_id IS 'Sincroniza o campo enum category com category_id da tabela categories';
COMMENT ON FUNCTION auto_sync_category_id IS 'Trigger para sincronizar automaticamente category e category_id em ambas as direções';
