-- Migração: Criar tabela de categorias (V3 - Corrigida com CAST)
...
-- Usando CAST explícito para converter para TEXT
INSERT INTO public.categories (name, active)
SELECT DISTINCT 
  TRIM(CAST(category AS TEXT)) as name,
  true as active
FROM public.businesses
WHERE category IS NOT NULL 
  AND TRIM(CAST(category AS TEXT)) != ''
  AND TRIM(CAST(category AS TEXT)) NOT IN (SELECT name FROM public.categories)
...
WHERE TRIM(CAST(b.category AS TEXT)) = c.name
  AND b.category IS NOT NULL
  AND TRIM(CAST(b.category AS TEXT)) != ''
...
-- NOTA: A coluna 'category' (TEXT/ENUM) será mantida temporariamente para compatibilidade
