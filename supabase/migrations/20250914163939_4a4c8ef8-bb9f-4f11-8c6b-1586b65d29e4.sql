...
-- Recreate get_random_businesses to include reviews_count
CREATE FUNCTION public.get_random_businesses(limit_count integer DEFAULT 5)
RETURNS TABLE(
  id uuid, 
  name text, 
  description text, 
...
-- Recreate get_featured_businesses to include reviews_count
CREATE FUNCTION public.get_featured_businesses(limit_count integer DEFAULT 5)
RETURNS TABLE(
  id uuid, 
  name text, 
  description text, 
...
-- Recreate get_public_businesses to include reviews_count
