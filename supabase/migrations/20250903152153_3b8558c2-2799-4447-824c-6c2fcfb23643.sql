-- FASE 2: Correção dos dados existentes e reestruturação do sistema baseado em CPF

-- Primeiro, vamos criar as funções para validação e formatação de CPF
...
-- Atualizar CPFs existentes para formato correto ou gerar CPFs fictícios para registros sem CPF
UPDATE public.profiles 
SET cpf = CASE 
    WHEN cpf IS NULL OR cpf = '' THEN 
        -- Gerar CPF fictício baseado no ID do usuário (temporário)
        LPAD(((hashtext(id::text) % 900000000) + 100000000)::text, 9, '0') || 
        LPAD(((hashtext(id::text) % 90) + 10)::text, 2, '0')
    WHEN validate_cpf(cpf) THEN 
        format_cpf(cpf)
    ELSE 
        -- Para CPFs inválidos, também gerar fictício
        LPAD(((hashtext(id::text) % 900000000) + 100000000)::text, 9, '0') || 
        LPAD(((hashtext(id::text) % 90) + 10)::text, 2, '0')
END;

-- Formatar todos os CPFs para o padrão correto
UPDATE public.profiles 
SET cpf = format_cpf(cpf)
WHERE cpf IS NOT NULL AND cpf !~ '^\d{3}\.\d{3}\.\d{3}-\d{2}$';

-- Agora podemos aplicar as constraints
...
-- Migrar dados existentes de email e telefone para user_contacts
INSERT INTO public.user_contacts (user_id, contact_type, contact_value, is_primary, verified)
SELECT 
  id as user_id,
  'email' as contact_type,
  email as contact_value,
  true as is_primary,
  true as verified -- Email já foi usado no sistema, consideramos verificado
...
SELECT 
  ...
  true as is_primary,
  false as verified -- Telefone não foi necessariamente verificado
FROM public.profiles 
...
-- Migrar dados existentes de cidade e estado para endereços
INSERT INTO public.user_addresses (user_id, address_type, street, city, state, is_primary)
SELECT 
  id as user_id,
  'residential' as address_type,
  'Endereço não especificado' as street,
  city,
  state,
  true as is_primary
FROM public.profiles 
